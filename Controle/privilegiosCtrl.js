            ...
            if (codigo > 0 && descricao) {
                ...
                privilegio.editar().then(() => {
                    ...
                    .catch((erro) => {
                        resposta.status(500).json({
                            "status": false,
                            "mensagem": "Não foi possível alterar o privilégio: " + erro.message
                        })
                    })
            }
            else {
                resposta.status(400).json(
                    {
                        "status": false,
                        "mensagem": "Informe corretamente todos os dados de um privilégio conforme documentação da API."
                    }
                )
            }
        }
        ...
        if (requisicao.method == 'DELETE') {
            ...
            if (codigo > 0) {
                ...
                resposta.status(400).json(
                    {
                        "status": false,
                        "mensagem": "Informe um código válido de um privilégio conforme documentação da API."
                    }
                )
            }
        }
            ...
            privilegio.consultar(codigo)
                ...
                .catch((erro) => {
                    resposta.status(500).json(
                        {
                            "status": false,
                            "mensagem": "Erro ao consultar privilégios:" + erro.message
                        }
                    )
                })
        }
